...
export function SystemStatusWidget({ className = "", showDetails = false }: SystemStatusWidgetProps) {
      ...
      <Card className={`${className} animate-pulse`}>
        <CardContent className="p-6">
          <div className="flex items-center justify-center">
            ...
            <span className="ml-2 text-primary">Carregando dados...</span>
          </div>
        </CardContent>
      </Card>
    );
  ...
  if (!showDetails) {
    // Compact widget view
    return (
      <Card className={`${className} bg-gradient-to-br from-primary via-primary/90 to-blue-800 text-white border-0 shadow-2xl overflow-hidden`}>
        <CardContent className="p-6 relative">
          ...
          <div className="relative grid grid-cols-2 gap-6">
            ...
            {/* Orders Today */}
            <div className="text-center">
              <div className="text-3xl sm:text-4xl font-bold text-primary/40 mb-1">
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        {/* Customers Card */}
        <Card className="bg-gradient-to-br from-primary/5 to-violet-100 border-primary/20 shadow-lg hover:shadow-xl transition-all duration-300">
          <CardContent className="p-4 sm:p-6">
            <div className="flex items-center justify-between mb-3">
              <div className="w-12 h-12 bg-gradient-to-br from-primary/50 to-primary rounded-xl flex items-center justify-center shadow-lg">
            ...
            <div className="space-y-1">
              <p className="text-2xl sm:text-3xl font-bold text-primary/90">
                {customerData?.total_customers || 0}
              </p>
              <p className="text-sm text-primary font-medium">Clientes Únicos</p>
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-primary/5 rounded-lg">
              <span className="text-sm font-medium text-primary/90">Taxa Clientes Fiéis</span>
              <span className="text-lg font-bold text-primary">{getRepeatCustomerRate().toFixed(1)}%</span>
            </div>
          </CardContent>
        </Card>
              ...
              {customerData?.top_customers?.slice(0, 5).map((customer, index) => (
                <div key={customer.whatsapp} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-gradient-to-br from-primary/50 to-primary rounded-full flex items-center justify-center text-white font-bold text-sm">
                      {index + 1}
                    </div>
                    <div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 bg-primary/5 rounded-lg">
              <p className="text-2xl font-bold text-primary">{customerData?.new_customers_this_month || 0}</p>
              <p className="text-sm text-primary/90 font-medium">Este Mês</p>
            </div>
          </div>
        </CardContent>
      </Card>
